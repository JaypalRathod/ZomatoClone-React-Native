...
import CustomGradiant from '@components/global/CustomGradiant';

const RecommndedList = () => {
    const { styles } = useStyles(cardStyles);

    const renderItem = ({ item }: any) => {
        return (
            <ScalePress style={styles.itemContainer} onPress={() => { navigate('RestaurantScreen', { item: item }) }}>
                <View style={styles.imageContainer}>
                    <Image source={{ uri: item?.imageUrl }} style={styles.itemImage} />
                    {item?.discount && (
                        <View style={styles.discountContainer}>
                            <CustomText fontFamily='Okra-Bold' fontSize={10} color={Colors.background}>
                                {item?.discount}
                            </CustomText>
                            {item?.discountAmount && (
                    ...
                    <CustomGradiant position={'bottom'} />
                </View>
                ...
                <View style={styles.itemInfo}>
                    ...
                    <View style={styles.flexRow}>
                        ...
                        <CustomText fontSize={9} color={Colors.lightText} fontFamily='Okra-Medium' numberOfLines={1} >
