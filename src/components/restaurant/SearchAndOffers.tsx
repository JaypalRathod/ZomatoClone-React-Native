        ...
        <View style={styles.container}>
            <View style={[styles.flexRowBetween, styles.padding]}>
                <TouchableOpacity style={styles.searchInputContainer} activeOpacity={0.8} >
                    <Icon name='search' iconFamily='Ionicons' color={Colors.active} size={RFValue(20)} />
                    <RollingBar interval={3000} defaultStyle={false} customStyle={styles.textContainer}>
                        {searchItems?.map((item, index) => {
                            return (
                                <CustomText style={styles.rollingText} key={index} fontSize={RFValue(16)}>
                                    {item}
                                </CustomText>
                            )
                        })}
                ...
                <TouchableOpacity style={styles.flexRowGap}>
                    <Icon name='silverware-fork-knife' iconFamily='MaterialCommunityIcons' color={Colors.lightText} size={RFValue(20)} />
                    ...
                    >
                        <View style={{
                            ...
                            >
                                ...
                                <CustomText style={styles.offerText}>{` item${summary?.totalItems > 1 ? 's' : ''}`}</CustomText>
                                <Icon name='arrow-right-circle' iconFamily='MaterialCommunityIcons' color={Colors.lightText} size={RFValue(20)} />


                            </TouchableOpacity>

                            <Animated.Text style={[styles.offerSubtitle, { transform: [{ scale: scaleAnim }] }]}>
                                {summary?.totalPrice > 500 ? 'Congratulations!, you get extra 15% OFF!' : `Add items worth ₹${Math.max(1, 500 - summary?.totalPrice)} more to get 15% OFF!`}
                            </Animated.Text>
